...
export function activate(context: vscode.ExtensionContext) {
    ...
    function updateUI(active: boolean = !!pythonProcess) {
    ...
    // --- Log Management ---
    function appendToRollingLog(message: string) {
    ...
    context.subscriptions.push(
        vscode.window.registerWebviewViewProvider('ag-autoaccept-view', viewProvider)
    );

    viewProvider.onToggle(() => {
        if (pythonProcess) stopEngine();
        else startEngine();
    });
...
class AutoAcceptViewProvider implements vscode.WebviewViewProvider {
    ...
    private _onToggleEmitter = new vscode.EventEmitter<void>();
    public readonly onToggle = this._onToggleEmitter.event;
    ...
    public updateState(active: boolean, count: number, logs: string[]) {
        if (this._view) {
